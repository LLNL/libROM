...
NNLSSolver::NNLSSolver()
  ...
  qr_residual_mode_(QRresidualMode::hybrid)
  {
    ...
    std::cout << "NNLSSolver init on rank " << d_rank << " out of " << d_num_procs << " processes" << std::endl;
  }
...
void NNLSSolver::normalize_constraints(Matrix& mat, Vector& rhs_lb, Vector& rhs_ub)
{
  ...
  MPI_Allreduce(MPI_IN_PLACE, rhs_avg_glob.getData(), m, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
  MPI_Allreduce(MPI_IN_PLACE, rhs_halfgap_glob.getData(), m, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
...
void NNLSSolver::solve_parallel_with_scalapack(const Matrix& mat, const Vector& rhs_lb, const Vector& rhs_ub, Vector& soln)
{
  int m = mat.numRows();
  ...
  for (unsigned int oiter = 0; oiter < n_outer_; ++oiter) {
    ...
    if (verbosity_ > 1 && d_rank == 0) {
      printf("%d %d %d %d %d %.15e %.15e\n", oiter, n_total_inner_iter, m, n_tot, n_glob, rmax, l2_res_hist(oiter));
      fflush(stdout);
    }
    ...
    MPI_Allreduce(MPI_IN_PLACE, &mumax_glob, 1, MPI_DOUBLE, MPI_MAX, MPI_COMM_WORLD);
    if (mumax_glob < mu_tol) {
      num_stalled = stalled_indices.size();
      MPI_Allreduce(&num_stalled, &num_stalled_glob, 1, MPI_INT, MPI_SUM, MPI_COMM_WORLD);
      if (num_stalled_glob > 0) {
        if (verbosity_ > 0 && d_rank == 0) {
          std::cout << "Lagrange multiplier is below the minimum threshold: mumax = " << mumax_glob << ", mutol = " << mu_tol << "\n" << " Resetting stalled indices vector of size " << num_stalled_glob << "\n";
        }
    ...
    MPI_Gather(&mumax, 1, MPI_DOUBLE, mu_max_array.data(), 1, MPI_DOUBLE, 0, MPI_COMM_WORLD);
	...
	for (int i=0; i<m; ++i)
      ...
      if (imax_proc == d_rank) {
        ...
        MPI_Send(mat.getData() + m*imax, m, MPI_DOUBLE, proc_to_recv, 189, MPI_COMM_WORLD);
      }
    }
	...
	for (int i = 0; i < n_glob; ++i)
	  ...
	  // TODO: there are many max/min computations. Refactor by implementing functions for this in Vector. Also for min/max index.
          ...
          alpha = soln_nz_glob(index_min)/(soln_nz_glob(index_min) - soln_nz_glob_up(index_min));
          // reupdate solution
    ...
    if (stalledFlag == 1) {
      ...
      if (d_rank == imax_proc) {
        ...
        if (verbosity_ > 2) {
          std::cout << "Adding index " << imax << " from processor " << imax_proc << " to stalled index list of size " << num_stalled << "\n";
        }
      }
    }
  ...
  */
  ...
  MPI_Scatterv(soln_nz_glob_psort.getData(), nnz_array.data(), nnz_cnt.data(), MPI_DOUBLE, soln_nz.getData(), n_nz_ind, MPI_DOUBLE, 0, MPI_COMM_WORLD);
  ...
  if (d_rank == 0 && verbosity_ > 0) {
    printf("NNLS solver: m = %d, n = %d, outer_iter = %d, inner_iter = %d", m, n_tot, n_outer_iter, n_total_inner_iter);
    if (exit_flag == 0) {
      printf(": converged\n");
    } else {
      printf("\n");
      printf("Warning: NNLS unconverged. Stalled = %d\n", exit_flag == 2);
      printf("resErr = %.8e vs tol = %.8e; mumax = %.6e vs tol = %.6e\n", rmax, const_tol_, mumax_glob, mu_tol);
    }
    fflush(stdout);
  }
}
