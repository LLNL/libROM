...
std::vector<ComplexEigenPair> NonSymmetricEigenSolve(const Matrix & A)
{
    ...
    double* e_real = new double [k];
    double* e_imaginary = new double [k];
    Matrix* ev_l = new Matrix(k, k, false);
    Matrix* ev_r = new Matrix(k, k, false);
    Matrix* A_copy = new Matrix(A);
    ...
    // Now call lapack to do the eigensolve.
    dgeev(&jobvl, &jobrl, &k, A_copy->getData(), &k, e_real, e_imaginary, ev_l->getData(),
          &k, ev_r->getData(), &k, work, &lwork, &info);
    ...
    for (int row = 0; row < k; ++row) {
        for (int col = row+1; col < k; ++col) {
            ...
            tmp = ev_l->item(row, col);
            ev_l->item(row, col) = ev_l->item(col,row);
            ev_l->item(col,row) = tmp;
        }
    }

    std::vector<ComplexEigenPair> eigenpairs;
    
    ComplexEigenPair eigenpair_right;
    eigenpair_right.ev_real = new Matrix(k, k, false);
    eigenpair_right.ev_imaginary = new Matrix(k, k, false);

    // Separate lapack eigenvector into real and imaginary parts
    for (int i = 0; i < k; ++i)
    {
        for (int row = 0; row < k; ++row) {
            eigenpair_right.ev_real->item(row, i) = ev_r->item(row, i);
        }
        if (e_imaginary[i] != 0)
        {
            for (int row = 0; row < k; ++row) {
                eigenpair_right.ev_real->item(row, i + 1) = ev_r->item(row, i);
                eigenpair_right.ev_imaginary->item(row, i) = ev_r->item(row, i + 1);
                eigenpair_right.ev_imaginary->item(row, i + 1) = -ev_r->item(row, i + 1);
            }
    ...
    for (int i = 0; i < k; i++)
    {
        eigenpair_right.eigs.push_back(std::complex<double>(e_real[i], e_imaginary[i]));
    }

    eigenpairs.push_back(eigenpair_right);

    ComplexEigenPair eigenpair_left;
    eigenpair_left.ev_real = new Matrix(k, k, false);
    eigenpair_left.ev_imaginary = new Matrix(k, k, false);

    // Separate lapack eigenvector into real and imaginary parts
    for (int i = 0; i < k; ++i)
    {
        for (int row = 0; row < k; ++row) {
            eigenpair_left.ev_real->item(row, i) = ev_l->item(row, i);
        }
        if (e_imaginary[i] != 0)
        {
            for (int row = 0; row < k; ++row) {
                eigenpair_left.ev_real->item(row, i + 1) = ev_l->item(row, i);
                eigenpair_left.ev_imaginary->item(row, i) = ev_l->item(row, i + 1);
                eigenpair_left.ev_imaginary->item(row, i + 1) = -ev_l->item(row, i + 1);
            }
    ...
    for (int i = 0; i < k; i++)
    {
        eigenpair_left.eigs.push_back(std::complex<double>(e_real[i], e_imaginary[i]));
    }

    eigenpairs.push_back(eigenpair_left);
