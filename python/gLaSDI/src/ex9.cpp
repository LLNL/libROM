...
// parameters for initial condition of Problem 3
double w1 = 1;
double w2 = 1;
...
int Mmax_iter = 100;   // maximum iterations in Msolver
const char *u_file = "./ex9-u_pred.gf"; // file path of given solutions
const char *res_file = "./ex9-residual.gf"; // file path to save calculated residual
...
int main(int argc, char *argv[])
{
   ...
   args.AddOption(&dt, "-dt", "--time-step",
                  "Time step.");
   args.AddOption(&w1, "-w1", "--w1",
                  "w1 of initial condition of Problem 3");
   args.AddOption(&w2, "-w2", "--w2",
                  "w2 of initial condition of Problem 3");
   args.AddOption(&res_ns, "-res_ns", "--residual_num_steps",
                  "Percentage of the time steps for residual evaluation");
   args.AddOption(&Mmax_iter, "-Mmax_iter", "--Msolver_max_iter",
                  "Maximum iterations in Msolver");
   ...
   if (!res_flag)
   {
    ...
    GridFunction tmp_u(&fes);
    tmp_u.SetFromTrueDofs(u);
    ofstream osol("ex9-u_"+to_string(0)+".gf");
    osol.precision(precision);
    tmp_u.Save(osol);
    osol.close();
   ...
   for (int ti = 0; !done; )
   {
      ...
      GridFunction tmp_u(&fes);
      tmp_u.SetFromTrueDofs(u);
      ofstream osol("ex9-u_"+to_string(ti)+".gf");
      osol.precision(precision);
      tmp_u.Save(osol);
      osol.close();
   }
...
//    cout << d1 << ' ' << d2 << endl;
   ...
   GridFunction res(&fes), u1(&fes), u2(&fes);
   double res_norm = 0.0, res_norm_tmp = 0.0;
   u1.Load(infile, d2); // u at t=0

   for (int i = 0; i < int(d1*res_ns)-1; i++){
       double dt_real = min(dt, t_final - t);
       ode_solver->Step(u1, t, dt_real); // true next u
