...
// Indicator files should be named identically to the corresponding row in TRAIN_LIST or TEST_LIST with appropriate "_idx.csv" or "_val.csv"
//   suffix.
// File structure:
...
//  9. run/OUT_DIR/INDICATOR_DIR/${INDICATOR}_idx.csv       -- (optional) each row specifies one indicator index
// 10. run/OUT_DIR/INDICATOR_DIR/${INDICATOR}_val.csv       -- (optional) each row specifies one indicator endpoint value
...
int main(int argc, char *argv[])
{
    ...
    args.AddOption(&indicator_dir, "-idc-dir", "--indicator-directory",
                   "Location of indicator files.");
    ...
    for (int idx_dataset = 0; idx_dataset < npar; ++idx_dataset)
    {
        ...
        if(!myid) std::cout << "training_par_list[" << idx_dataset << "] = " << training_par_list[idx_dataset] << std::endl;
    ...
    for (int idx_dataset = 0; idx_dataset < npar; ++idx_dataset)
    {
        if(!myid) std::cout << "Parameter " << idx_dataset << ":" << std::endl;
        for(int par_order = 0; par_order < dpar; ++par_order)
        {
            if(!myid) std::cout << training_par_vectors[idx_dataset]->item(par_order) << ", ";
        }
        if(!myid) std::cout << std::endl;
    }
...
*/
    ...
    if (online)
    {
        for (int idx_dataset = 0; idx_dataset < npar; ++idx_dataset)
        {
            ...
            {
                std::cout << "twep = ";
                for(int ii  = 0; ii < numWindows+1; ii++){ std::cout << twep->item(ii) << ", ";}
                std::cout << std::endl;
            }
        }
        par_dir_list.clear();
    ...
    if (online || predict)
    {
        ...
        {            
            ...
            {
                std::cout << "curr_par = :";
                for(int ii = 0; ii < dpar; ii++)
                {
                    std::cout << curr_par->item(ii) << ", ";
                }
                std::cout << std::endl;
            }
            CAROM::Vector* twep;
            ...
            for (int idx_snap = snap_bound[0]; idx_snap <= snap_bound[1]; ++idx_snap)
            {
                ...
                if (min_idx_snap == -1)
                {
                    if(!myid) std::cout << "line ~789: twep->item(0) = " << twep->item(0) << ", tval = " << tval << std::endl;
                    if (tval >= twep->item(0))
                    {
                        min_idx_snap = idx_snap;
                        if (myid == 0)
                                ...
                                double closest_rbf_val = 0.9)
{
    ...
    std::cout << "line ~1001: desired point = ";
    for(int ii = 0; ii < desired_point->dim(); ii++)
    {
        std::cout << desired_point->item(ii) << ",";
    }
    std::cout << std::endl;
    std::cout << "rbf = " << rbf << std::endl;

    
    for(int ii = 0; ii < parameter_points.size(); ii++)
    {
        std::cout << "line ~1011: parameter points[" << ii << "] = ";
        for(int jj = 0; jj < parameter_points[ii]->dim(); jj++)
        {
            std::cout << parameter_points[ii]->item(jj) << ",";
        }
        std::cout << std::endl;
    ...
    std::cout << "line ~1004: epsilon = " << epsilon << std::endl;
    
    std::vector<double> rbf_val = obtainRBFToTrainingPoints(parameter_points, "IDW",
                                  rbf, epsilon, desired_point);

    std::cout << "line ~1007: rbf_val = ";
    for(int ii = 0; ii < rbf_val.size(); ii++)
    {
        std::cout << rbf_val[ii] << ",";
    }
    std::cout << std::endl;
