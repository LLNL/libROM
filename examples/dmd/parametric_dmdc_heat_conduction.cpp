...
int main(int argc, char *argv[])
{
    ...
    u_gf.SetFromTrueDofs(u);
    {
        ...
        mesh_name << outputPath << "/parametric_dmdc_heat_conduction_" << to_string(
                      amp_in) << "_" << to_string(amp_out) << "-mesh." << setfill('0') << setw(6) << myid;
        sol_name << outputPath << "/parametric_dmdc_heat_conduction_" << to_string(
                     amp_in) << "_" << to_string(amp_out) <<  "-init." << setfill('0') << setw(6) << myid;
...
//            ts.push_back(t);
    ...
    //     using GLVis: "glvis -np <np> -m heat_conduction_dmdc-mesh -g heat_conduction_dmdc-final".
    {
        ...
        sol_name << outputPath << "parametric_dmdc_Heat_Conduction" << to_string(amp_in) << "_" << to_string(amp_out) << "-final." << setfill('0') << setw(6) << myid;
        ofstream osol(sol_name.str().c_str());
        osol.precision(precision);
        u_gf.Save(osol);
//        ostringstream sol_name;
...
//         probably will need to change a lot here
        if (online)
        {
            ...
            while (fin >> curr_param)
            {
                ...
                dmd_paths.push_back(outputPath + "/" + to_string(curr_amp_in) + "_" + to_string(curr_amp_out) );
            ...
            dmd_u->project(init,param_vectors);
