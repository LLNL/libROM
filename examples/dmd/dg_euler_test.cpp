...
//
// Output 1:
//   Relative error of NonuniformDMD density (dens) at t_final: 0.1 is 0.00015279217
//   Relative error of NonuniformDMD x-momentum (x_mom) at t_final: 0.1 is 2.9074268e-05
//   Relative error of NonuniformDMD y-momentum (y_mom) at t_final: 0.1 is 8.9542312e-05
//   Relative error of NonuniformDMD energy (e) at t_final: 0.1 is 6.8687185e-05
//
// Command 2:
//   mpirun -np 8 dg_euler -p 2 -rs 2 -rp 1 -o 1 -s 3 -tf 0.1 -visit
//
// Output 2:
//   Relative error of NonuniformDMD density (dens) at t_final: 0.1 is 2.1881846e-06
//   Relative error of NonuniformDMD x-momentum (x_mom) at t_final: 0.1 is 4.3873307e-05
//   Relative error of NonuniformDMD y-momentum (y_mom) at t_final: 0.1 is 0.0026632618
//   Relative error of NonuniformDMD energy (e) at t_final: 0.1 is 2.3054118e-06
//
// Command 3:
//   mpirun -np 8 dg_euler -p 2 -rs 2 -rp 1 -o 1 -s 3 -visit
//
// Output 3:
//   Relative error of NonuniformDMD density (dens) at t_final: 2 is 1.9677493e-05
//   Relative error of NonuniformDMD x-momentum (x_mom) at t_final: 2 is 0.00011781827
//   Relative error of NonuniformDMD y-momentum (y_mom) at t_final: 2 is 0.00017120499
//   Relative error of NonuniformDMD energy (e) at t_final: 2 is 1.9836223e-05
//
// =================================================================================
//
...
int main(int argc, char *argv[])
{
    ...
    args.AddOption(&ef, "-ef", "--energy_fraction",
                   "Energy fraction for NonuniformDMD.");
    args.AddOption(&rdim, "-rdim", "--rdim",
                   "Reduced dimension for NonuniformDMD.");
    ...
    CAROM::NonuniformDMD dmd_dens(u_block.GetBlock(0).Size());
    CAROM::NonuniformDMD dmd_x_mom(u_block.GetBlock(1).Size());
    CAROM::NonuniformDMD dmd_y_mom(u_block.GetBlock(2).Size());
    CAROM::NonuniformDMD dmd_e(u_block.GetBlock(3).Size());
    ...
    if (rdim != -1)
    {
        if (mpi.WorldRank() == 0)
        {
            std::cout << "Creating NonuniformDMD with rdim: " << rdim << std::endl;
        }
        dmd_dens.train(rdim);
    ...
    else if (ef != -1)
    {
        if (mpi.WorldRank() == 0)
        {
            std::cout << "Creating NonuniformDMD with energy fraction: " << ef << std::endl;
        }
        dmd_dens.train(ef);
        dmd_x_mom.train(ef);
        dmd_y_mom.train(ef);
    ...
    if (mpi.WorldRank() == 0)
    {
        std::cout << "Predicting density, momentum, and energy using NonuniformDMD" << std::endl;
    }
    ...
    if (mpi.WorldRank() == 0)
    {
        ...
        std::cout << "Relative error of NonuniformDMD energy (e) at t_final: " << t_final << " is " << tot_diff_norm_e / tot_true_solution_e_norm << std::endl;
        printf("Elapsed time for solving FOM: %e second\n", fom_timer.RealTime());
        printf("Elapsed time for training NonuniformDMD: %e second\n", dmd_training_timer.RealTime());
        printf("Elapsed time for predicting NonuniformDMD: %e second\n", dmd_prediction_timer.RealTime());
    }
