...
//
// Command 1:
//  wave_equation_dmdc -o 4 -tf 5 -nwinsamp 25
//
// Output 1:
...
public:
    WaveOperator(FiniteElementSpace &f, FunctionCoefficient &s, 
                    Array<int> &ess_bdr,double speed, double sigma);
...
WaveOperator::WaveOperator(FiniteElementSpace &f,FunctionCoefficient &s,
                           Array<int> &ess_bdr, double speed, double sig)
    : SecondOrderTimeDependentOperator(f.GetTrueVSize(), 0.0), fespace(f), src_func(s),M(NULL),
      K(NULL),
...
int main(int argc, char *argv[])
{
    ...
    WaveOperator oper(fespace,s, ess_bdr, speed, sigma);
    ...
    dmd_u.push_back(new CAROM::DMDc(u.Size(),1, dt));
    ...
    for (int ti = 1; !last_step; ti++)
    {
        ...
        if (last_step || (ti % windowNumSamples) == 0)
        {
            ...
            if (rdim != -1) {
                ...
                cout << "Creating DMDc with energy fraction: " << ef << " at window index: " <<
                     curr_window << endl;
