...
TEST_CASE("Copying column major array to distributed and back",
          "[data-movement]")
{
    ...
    for (int rank = 0; rank < mpi_size(); ++rank)
    {
        SECTION(std::string("default_copy_from") + std::to_string(rank))
        {
            ...
            {
                cout << "Testing column major copy with default blocks from rank "
                     << rank << '\n';
            ...
            LocalMatrix(diff.get(), global_m, global_n, rank, false, COL_MAJOR) =
                A(rowrange(1, global_m), colrange(1, global_n));
            if (mpi_rank() == rank && NOISY_TESTS)
            {
                cout << "Local matrix as retrieved on rank " << rank << ":\n";
                print_local_mat(diff.get(), global_m, global_n, COL_MAJOR);
                cout << "\n\n";
            }
