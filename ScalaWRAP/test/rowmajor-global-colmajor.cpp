...
TEST_CASE("Copying row major array to distributed and back to column major",
          "[data-movement]")
{
        ...
        SECTION(std::string("default_copy_from") + std::to_string(rank))
        {
            ...
            if (mpi_rank() == rank && NOISY_TESTS)
            {
                cout << "Testing row major copy with default blocks from rank "
                     << rank << '\n';
                cout << "Testing copy from row major to global back to col major\n";
            ...
            if (mpi_rank() == rank)
            {
                for (int i = 0; i < global_n; ++i)
                {
                    ...
                    {
                        diff[j * global_n + i] -= local_data[i * global_m + j];
                    }
                }
            }
