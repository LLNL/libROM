...
TEST_CASE("Copying column major array to distributed and back to row major",
          "[data-movement]")
{
    ...
    for (int rank = 0; rank < mpi_size(); ++rank)
    {
        SECTION(std::string("default_copy_from") + std::to_string(rank))
        {
            ...
            {
                cout << "Testing column major copy back to row major with default blocks from rank "
                     << rank << '\n';
                cout << "Local matrix on rank " << rank << ":\n";
                print_local_mat(local_data.get(), global_m, global_n, COL_MAJOR);
                cout << "\n\n";
            }
            LocalMatrix localmat(local_data.get(), global_m, global_n, rank, false, COL_MAJOR);
            ...
            if (mpi_rank() == rank)
            {
                for (int i = 0; i < global_n; ++i)
                {
                    ...
                    {
                        diff[i * global_m + j] -= local_data[i * global_n + j];
                    }
                }
            }
    ...
    for (int rank = 0; rank < mpi_size(); ++rank)
    {
        SECTION(std::string("smallblock_copy_from" + std::to_string(rank)))
        {
            ...
            if (mpi_rank() == rank && NOISY_TESTS)
            {
                cout << "Testing copy with 2 x 2 blocks from colmajor then back to rowmajor from " << rank << '\n';
                cout << "Local matrix on rank " << rank << ":\n";
                print_local_mat(local_data.get(), global_m, global_n, COL_MAJOR);
                cout << "\n\n";
            }
    ...
    for (int rank = 0; rank < mpi_size(); ++rank)
    {
        SECTION(std::string("randblock_copy_w_offset_from_" + std::to_string(rank)))
        {
            ...
            if (mpi_rank() == rank && NOISY_TESTS)
            {
                ...
                cout << "Testing copy from colmajor to global back to rowmajor\n";
                cout << "Testing with random rowsrc and colsrc\n";
                cout << "Local matrix on rank " << rank << ":\n";
                print_local_mat(local_data.get(), global_m, global_n, COL_MAJOR);
                cout << "\n\n";
    ...
    for (int rank = 0; rank < mpi_size(); ++rank)
    {
        SECTION(std::string("randblock_copy_w_offset_and_random_map_from_" + std::to_string(rank)))
        {
            ...
            int pi, pj;
            int host = pids[0];
            std::tie(pi, pj) = get_random_coord(ctxt, host);
            int bs[2] = { 0 };
            if (mpi_rank() == 0) {
                bs[0] = random_between(1, global_m);
                bs[1] = random_between(1, global_n);
            ...
            {
                cout << "Testing copy with random blocksize from " << rank << '\n';
                cout << "Testing copy from colmajor to global to rowmajor\n";
                cout << "Testing with random rowsrc and colsrc\n";
