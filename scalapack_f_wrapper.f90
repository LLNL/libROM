...
the associated header file 'scalapack_wrapper.h'
...
if (.not. initialized) then
    ...
    if (GLOBAL_CTXT .eq. -1) then
        ...
        !*******************************************************************************
! Subroutine:
        initialize_matrix(A, m, n, nprow, npcol, mb, nb) bind(C)
        !
! Synopsis:
        Initialize a SLPK_Matrix structure with a processor grid and
        ! allocated storage.
        !
! Detail:
        `A` is treated as a new, uninitialized object and contents are
! replaced blindly. This initializer performs a few tasks:
        it initializes MPI if
        ! a call to MPI_Init has not been made yet;
it checks that the number of
                                  ...
                                  call sl_init( A%ctxt, nprow, npcol )
                                      ...
                                      A%mm = numroc(m, mb, A%pi, 0, nprow)
                                             A%mn = numroc(n, nb, A%pj, 0, npcol)
                                                    ...
                                                    !*******************************************************************************
                                        ! Subroutine:
                                                    make_similar_matrix(dst, m, n, ctxt, mb, nb) bind(C)
                                                    !
                                        ! Synopsis:
                                                    Creates an m x n matrix in `dst` with the given BLACS parameters.
                                                    !
                                        ! Detail:
                                                    This is provided as a tool to easily re-use the same BLACS context as
                                                    ...
                                                    ! =========
                                                !   - dst:
                                                            An SLPK_Matrix (pointer to, from C)
                                                !   - m, n:
                                                            Dimensions of the matrix to create.
                                                !   - ctxt:
                                                            BLACS context to re-use.
                                                !   - mb, nb:
...
dst%ctxt = ctxt
           ...
           dst%mm = numroc(m, mb, dst%pi, 0, dst%nprow)
                    dst%mn = numroc(n, nb, dst%pj, 0, dst%npcol)
                             ...
                             !*******************************************************************************
                             ! Subroutine:
                             wrapper_finalize() bind(C)
                             !
                             ! Synopsis:
                             Release the global BLACS context initialized by this wrapper and
                             ...
                             if (GLOBAL_CTXT .ne. -1) then
                                 ...
                                 !*******************************************************************************
                             ! Subroutine:
                                 release_context(A) bind(C)
                                 !
                             ! Synopsis:
                                 Free the resources used by A's associated BLACS context.
                                 !
                             ! Detail:
                                 *WARNING* Make sure the context associated with A is not still in use
